...
// CHECK-clock-DAG: [MUST-REPORT] Error: from: call {{[MPI_Isend@|MPI_Recv@].*}}: Found concurrent mpi calls while MPI_THREAD_SERIALIZED was requested.
// CHECK-counter-DAG: [MUST-REPORT] Error: from: call {{[MPI_Isend@|MPI_Recv@].*}}: Multiple threads call MPI functions simultaneously while you required MPI_THREAD_SERIALIZED.
