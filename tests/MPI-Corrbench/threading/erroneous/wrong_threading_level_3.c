...
// RUN: env OMP_NUM_THREADS=2 mustrun --must:mpimode MPMD --must:instant-logging warning --must:openmp --must:layout %root-dir/%omp-layout \
...
// CHECK-clock-DAG: [MUST-REPORT] Error: from: call {{[MPI_Send@|MPI_Recv@].*}}: Found concurrent mpi calls while MPI_THREAD_SERIALIZED was requested.
// CHECK-counter-DAG: [MUST-REPORT] Error: from: call {{[MPI_Send@|MPI_Recv@].*}}: Multiple threads call MPI functions simultaneously while you required MPI_THREAD_SERIALIZED.
