...
// RUN: env OMP_NUM_THREADS=2 mustrun --must:output stdout --must:instant-logging warning --must:openmp --must:layout %root-dir/%omp-layout \
...
// CHECK-clock-DAG: [MUST-REPORT] Warning: from: call {{.*}}: Found concurrent MPI receive and probe
